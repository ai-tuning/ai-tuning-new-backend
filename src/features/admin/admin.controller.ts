...
export class AdminController {
    ...
    @Get('winols-setting/:adminId')
    async getWinolsSetting(@Param('adminId') adminId: Types.ObjectId) {
        const enabledWinols = await this.adminService.getWinolsSetting(adminId);
        return enabledWinols;
    }

    @AccessRole([RolesEnum.ADMIN, RolesEnum.SUPER_ADMIN])
    @Patch('winols-setting/:adminId')
    async updateWinolsSetting(@Param('adminId') adminId: Types.ObjectId, @Body() body: { enabledWinols: boolean }) {
        const enabledWinols = await this.adminService.updateWinolsSetting(adminId, body.enabledWinols);
        return { message: 'Ai assist updated successfully', data: enabledWinols };
    }
}
