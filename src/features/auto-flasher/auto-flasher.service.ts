...
export class AutoFlasherService {
    ...
    async decode(autoFlasherDecodeDto: AutoFlasherDecodeDto) {
        ...
        const rootpath = this.pathService.getFileServicePath(
            autoFlasherDecodeDto.adminId,
            autoFlasherDecodeDto.documentId,
        );

        let unzippedFilePath = path.join(rootpath, 'extracted_files' + autoFlasherDecodeDto.uniqueId);
        ...
        try {
            ...
            if (response.data.status === 'OK') {
                ...
                const newDecodedFilePath = path.join(
                    this.pathService.getFileServicePath(autoFlasherDecodeDto.adminId, autoFlasherDecodeDto.documentId),
                    newFileName,
                );
    ...
    async encode(autoFlasherEncodeDto: AutoFlasherEncodeDto) {
        ...
        const basePath = path.join(
            this.pathService.getFileServicePath(autoFlasherEncodeDto.adminId, autoFlasherEncodeDto.documentId),
        );
