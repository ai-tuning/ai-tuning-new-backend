...
export class Kess3Service {
    ...
    private async downloadDecodeFile(
        ...
        const rootpath = this.pathService.getFileServicePath(adminId, documentId);

        if (!fs.existsSync(rootpath)) await fs.promises.mkdir(rootpath);

        // Path where the file will be saved
        const filePath = path.join(rootpath, data.name);
    ...
    async decodeFile(decodeFileServiceDto: DecodeKess3FileDto) {
        ...
        try {
            ...
            const decodedFile = await this.downloadDecodeFile(
                decodeFileServiceDto.adminId,
                decodeFileServiceDto.documentId,
