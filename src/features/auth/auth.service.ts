...
export class AuthService {
    ...
    async registration(registrationDto: RegistrationDto, username?: string) {
        if (username && username !== 'dyno-files.org') {
    ...
    async resendCode(email: string, verificationType: VerificationEmailEnum) {
        ...
        //get admin id
        let adminId = new Types.ObjectId(process.env.SUPER_ADMIN_ID);

        if (user.admin) {
            adminId = user.admin;
        } else if (user.customer) {
            const customer = await this.customerService.findByIdAndSelect(user.customer, ['admin']);
            adminId = customer.admin;
        } else if (user.employee) {
            const employee = await this.employeeService.findByIdAndSelect(user.employee, ['admin']);
